...
| Nancy dust and Tommy snowman frame-offset tables | `src/ending/ending_credits_effects.c` | SK2 `src/credits/` subsystem | Same scene family, divergent effects | Full SK1 SHA-1 gate; 16 bytes source-owned | SK1 uses two four-frame `0, 1, 2, 1` cycles. No one-to-one SK2 effect table was located, so only SK1 target bytes establish these definitions. |
| Tommy ending shadow quad `D_800B8100` | `src/ending/ending_credits_tommy.c` | SK2 `src/credits/credits_controller.c` | Same character-credits family, divergent renderer | Full SK1 SHA-1 gate; 64 bytes source-owned | SK1 uses a 32x32 white four-vertex textured quad. SK2's character credits use a different entity/renderer path, so SK1 target bytes establish the vertex definition. |
| Audio-thread state globals | `src/libmus/aud_thread.c` | SK2 libmus/audio runtime | Shared libmus subsystem, divergent scheduler integration | Full SK1 SHA-1 gate; 32 bytes source-owned | SK1 owns five zero-initialized counters/pointers plus underrun state initialized to one. The SK2 corpus confirms subsystem boundaries but does not expose a one-to-one data object. |
| Character raw, texture, and model ROM ranges | `src/race/scene/race_scene_setup.c` | SK2 `src/race/race_session.c` and character initialization | Same race/character asset boundary, divergent ROM layout | Full SK1 SHA-1 gate; 144 bytes source-owned | SK1 owns three six-character start/end tables expressed through named ROM symbols. Never transfer SK2 offsets; its corpus supports only the subsystem and ownership model. |
| Race-player rumble pattern tables | `src/race/player/race_player_rumble.c` | `gRumblePatterns` and rumble update in `src/race/race_main.c` | Same feature, divergent encoding | Full SK1 SHA-1 gate; 64 bytes source-owned | SK1 stores four byte-per-frame patterns; SK2 uses packed bitmasks and per-effect durations. The SK2 logic confirms intent but not SK1 table bytes. |
| Fixed-point identity and shadow matrix templates | `src/math/fixed_point_math.c` | `identityMatrix`, `gIdentityMatrix32`, and geometry helpers in `src/math/geometry_data.c` and `src/math/geometry.c` | Same math/transform subsystem, divergent scale and packed templates | Full SK1 SHA-1 gate; 160 bytes source-owned | SK1 uses 4.12 rotation values and owns an N64 s15.16 identity matrix plus a player-shadow matrix template. SK2 uses 8.13 transform values and does not provide byte-identical packed templates. |
